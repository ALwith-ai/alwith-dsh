...
- **Composition is deterministic**: the per-preset roster is fixed in `src/plugins.ts` (mounted by `src/compose.ts`); users only toggle/configure rows through the overrides file. No dsh loader/profile/patch trees, no HMR — the sidecar must run on Bun.
